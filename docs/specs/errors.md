...
| OSM_COAST_ORIENTATION | O4_Vector_Utils.py:941-947 | Boundary walk loops 1000 times: a coastline way is drawn with water on the left | Same: empty sea polygon, tile without sea | blocking | stop | Reverse the faulty coastline way in OpenStreetMap, then build this tile again. |
...
| DSF_POOL_OVERFLOW | O4_DSF_Utils.py:1150-1160 | More than 65 535 vertices land in one 16-bit pool; the context gives the corner of the fullest pool (`at`), where the mesh piled them | `struct.pack("<H")` raises inside the DSF thread: no DSF, GUI frozen | blocking | stop | Internal error; report the tile and parameters (`osxp debug report`). |
...
| CFG_SIMBRIEF_PLAN_EMPTY | new | The last plan of that user says nowhere it goes (no airports, no navigation log with coordinates) | – | blocking | stop | Generate a flight plan on simbrief.com, then try again. |
