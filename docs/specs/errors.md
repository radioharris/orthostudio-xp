...
| ZONE_TOO_SMALL | new | A zone of the plan holds no mesh cell centre, so its detail level raises nothing (the grid is about 850 m at `mesh_zl` 19, Ortho4XP's own rule) | No counterpart: Ortho4XP says nothing either | info | continue | Draw it about a kilometre across, or leave it as it is: its colours still apply to exactly the shape you drew. Only the detail level follows the ground grid, which is how X-Plane's terrain is cut. |
