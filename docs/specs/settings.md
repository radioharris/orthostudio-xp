...
| `simbrief_user` | str or absent (`None` = the button is refused before anything is asked) | - | `None` | - (OrthoStudio XP) | The SimBrief name, or pilot ID, whose last flight plan step 1 of the Plan draws to choose the squares a route flies over (user request, 2026-09-19; `api.md` `GET /api/simbrief`). It is the one thing of the user that leaves the machine, and only when that button is pressed; *Default values* keeps it, as it keeps the folders of this computer. |
...
| `osm_library` | str | - | empty | - | The baked library read before the public servers; empty switches it off. |
| `osm_library_token` | str | - | empty | - | The key that library opens with, sent with every request, manifest included. |
| `osm_prepared_public` | bool | - | true | - | Whether another project's library may answer, and only for the tiles our manifest says we verified. |
...
| `patches_dir` | str | - | `""` | – | Folder of hand-made mesh patches (`<tile>/*.patch.osm`, files written with JOSM; patches published for Ortho4XP fit as they are, in their `Patches/<10° cell>/<tile>` tree; the tile's own directory will do too). Empty: `$OSXP_HOME/patches`, which `osxp serve` makes at start. |
...
| `photo_brightness` | float `[-0.5, 0.5]` | - | 0 | – | Brightness of the photo, used when `essential.photo_look` is `custom`. |
