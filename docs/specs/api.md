...
| `GET /api/simbrief` | – | the last flight plan of the SimBrief name saved in Settings (`essential.simbrief_user`), reduced to a line the Plan draws: `{from, to, points: [{ident, name, lat, lon}]}`, the navigation log included, so the line follows the real path and not a straight leg. The engine asks, not the page, and only when the button is pressed: the name is the one thing that leaves the machine, nothing is kept on disk. `400 `CFG_SIMBRIEF_USER_MISSING`` with no name (nothing is asked of the service), `404 `CFG_SIMBRIEF_USER_UNKNOWN`` for a name they refuse (their HTTP 400), `404 `CFG_SIMBRIEF_PLAN_EMPTY`` for a plan that says nowhere, `502 `NET_CONNECTION_FAILED`` when the service does not answer |
