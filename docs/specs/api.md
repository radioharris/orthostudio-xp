...
| `GET /api/providers` | – | `[{code, name, max_zl, attribution, terms_url, alive, extent, extent_bounds, same_as, custom}]` in the registry's order (Bing Maps, then Esri), the user's sources last with their `url_template` and `cache`, the folder their images are kept in, which the page puts in their map tile URLs (`imagery-providers.md` 4.1); `extent` is the country in English, `null` for the whole world (`alive` is `null`: no probe offline) |
...
| `GET /api/photo-sample` | `provider`, `lat`, `lon`, `zl?` (10-18, capped by the source) | one tile of that source's own grid **as an image** (`image/jpeg`, `image/png`, `image/webp`; `Cache-Control: private, max-age=3600`), for the colours preview of Settings (a user asked for the colours, 2026-09-18); `422 CFG_VALUE_INVALID` for an unknown source, `IMG_TILE_PLACEHOLDER` when the source answers its own "no imagery" tile (Bing does that over open water rather than a 404, and the page said "the ground where the map is looking" over it), `NET_UNEXPECTED_STATUS` when nothing came back |
...
| `GET /api/basemap/{path}` | – | one piece of it: a vector tile, a glyph range, a sprite, the low-zoom raster, or a tile source (JSON, rewritten the same way). Kept under `<data folder>/mapcache/openfreemap`, which *Free space* empties with the map background; a week of browser cache for the binary pieces. `502` with a degraded `NET_CONNECTION_FAILED` when the service does not answer: the aerial imagery is untouched |
| `GET /api/airports/in` | `west`, `south`, `east`, `north`, `limit?` (1-2000, 400 by default), `icao_only?` | the airports of that rectangle for the map's layer: `[{icao, name, lat, lon, kind}]`, explicit ICAO codes first, and with `icao_only` those alone (the map asks for it: the others carry an identifier of X-Plane's own, unreadable on a map), read from the shipped index (no request leaves the machine); a rectangle crossing the antimeridian is read as two; `503 SYS_RESOURCE_MISSING` without an index |
| `GET /api/airports/{icao}` | – | one airport or 404 |
| `GET /api/flightplan/simbrief` | `?radius_km=15` | the last flight plan of the SimBrief name saved in Settings (`essential.simbrief_user`: a name goes as `username`, encoded, a pilot ID as `userid`), as squares to build (`flight-plan.md` 4): `{from, to, points, path, squares: {ends, along}, left_out, length_km, bounds, radius_km, scenery_checked}`. The line is the plan's points joined by great circles, cut at the antimeridian; `ends` are the squares within `radius_km` of the departure and the arrival, `along` every other square within it of the route, in the order flown; the squares X-Plane has no scenery for are left out and counted. The engine asks, not the page, and only when the button is pressed: the name is the one thing that leaves the machine, nothing is kept on disk. Errors in the engine's one shape: 400 `CFG_SIMBRIEF_USER_MISSING` with no name (nothing is asked of the service), 404 `CFG_SIMBRIEF_USER_UNKNOWN` for a name they refuse (their HTTP 400), 404 `CFG_SIMBRIEF_PLAN_EMPTY` for a plan that says nowhere, 502 `NET_SIMBRIEF_FAILED` when the service does not answer or answers what cannot be read; 422 for a radius outside 0 to 300 km |
| `POST /api/flightplan` | `{from, to, points: [{ident, name, lat, lon}], radius_km}` | a route the page kept, answered as `GET /api/flightplan/simbrief` answers: its squares and its line computed again by the rules of this version (`flight-plan.md` 5), SimBrief not asked and no name needed. The page keeps the route, never the squares: kept whole, a plan read before the corridor came still chose the line's squares alone after the update (2026-09-25). Nothing else is taken: 422 for another field (squares included), fewer than 2 points or more than 402, a point off the globe, a radius outside 0 to 300 km |
...
| `DELETE /api/jobs/{id}` | – | `{job_id, removed: true}`: that one finished job leaves the list, with its `<id>.json` and `<id>.jsonl`, so a restart does not list it again; the tiles it built are not touched. 404 when there is no such job; 409 `SYS_BUSY` while it is running or waiting its turn, saying to cancel it first. The decision is taken under the manager's lock, so a job that starts between a caller's look and the call is not taken from under its thread |
...
- by the same installation as recent as this one, running the same files (its `/api/status` gives
  an `api_level` at least this one's, `engine.root` this one's package folder and `engine.code`
  the mark of this one's files, `orthostudio.codemark`): its page opens, nothing starts. Opening
  the app twice shows the running OrthoStudio XP;
- by the same installation started before its files changed (another `engine.code`: a checkout
  after a pull, another branch or an edit): it is asked to quit like an older one, below. An
  engine loads its code when it starts: the checkout opened again showed the engine started
  before the change, the new page on the old code, and a pilot tested a fix that was not running
  (2026-09-25). The mark is the name, size and time of change of each `.py` of the package, a
  few milliseconds; the page's files are not in it, since they are read from disk at each load;
