...
| `GET /api/photo-sample` | `provider`, `lat`, `lon`, `zl?` (10-18, capped by the source) | one tile of that source's own grid **as an image** (`image/jpeg`, `image/png`, `image/webp`; `Cache-Control: private, max-age=3600`), for the colours preview of Settings (a user asked for the colours, 2026-09-18); `422 CFG_VALUE_INVALID` for an unknown source, `NET_UNAVAILABLE` when nothing came back |
