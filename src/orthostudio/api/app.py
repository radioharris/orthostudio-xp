...
squares alone), 22 = ``DELETE /api/jobs/{id}`` (one finished build leaves the list), 25 = the
setting ``expert.decal`` (an older engine refuses a settings document that holds it). A page
